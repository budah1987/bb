...
describe("app keybindings", () => {
  ...
  it("serves validated explicit defaults from system config", async () => {
    await withTestHarness(async (harness) => {
      ...
      // BBamir's Alt space jumps bind digit keys, so the composer cycling
      // chords (letter keys) cannot be shadowed by an earlier binding for the
      // same chord.
      expect(
        assignedDefaultKeybindings
        ...
        ...SPACE_JUMP_APP_COMMAND_IDS.map((command) => ({
          command,
          shift: false,
        })),
