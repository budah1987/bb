...
export function useDraftAttachmentUploads({
  ...
  const handleAttachFiles = useCallback(
    async (files: File[]) => {
      ...
      // BBamir rejects formats the server cannot store before any upload
      // starts, so an unsupported pick reports a format error instead of a
      ...
      if (accepted.length === 0) {
        setOperation((current) => ({
          ...
          pendingCount:
            current.targetKey === capturedTargetKey ? current.pendingCount : 0,
