...
describe("ThreadRow", () => {
  ...
  // BBamir keeps the shortcut pill subordinate to row indicators, rejecting
  // upstream #1297. A row already carrying a signal stays uncluttered.
  it("keeps a plugin status instead of the keyboard shortcut", () => {
    setPluginThreadRowStatus("thr_test", "composer-status-test", {
      icon: "AiContentGenerator01",
      label: "Plugin improving draft",
    });
  ...
  it("keeps a split mini-map instead of the keyboard shortcut", () => {
  ...
  it("keeps an active indicator instead of its Command shortcut", () => {
    renderThreadRow({
      shortcutKey: "3",
      thread: createThread({
        status: "active",
    ...
    // The pill is hidden, but the shortcut stays bound to the row.
    expect(
      screen
