...
describe("host-daemon command schemas", () => {
  ...
  // turn-qualified fileChange ids and direct publication payloads (89-91), and
  // upstream's ACP context-window usage outside a turn (through 95). A stock
  // upstream daemon reports 95 and would satisfy a `>= 95` check while missing
  // the BBamir payloads, so the merged build claims 96 to force every older
  // daemon on either lineage to update before it connects.
