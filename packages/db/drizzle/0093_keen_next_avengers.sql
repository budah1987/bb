...
CREATE INDEX `browser_annotations_thread_status_updated_idx` ON `browser_annotations` (`thread_id`,`status`,`updated_at`);--> statement-breakpoint
CREATE INDEX `browser_annotations_thread_tab_updated_idx` ON `browser_annotations` (`thread_id`,`browser_tab_id`,`updated_at`);